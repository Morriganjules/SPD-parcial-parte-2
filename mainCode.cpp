...
void loop()
{
  //Se lee la fuerza que se ejerce en el sensor y determina la velocidad con la cual cambiaran los numeros.
  lecturaSensor = analogRead(SENSOR);
  int fuerza = map(lecturaSensor,0,466,0.0,10.0);
  determineVelocity(fuerza);

  //La variable value hace referencia al valor que devuelve el switch y se utiliza para definir tanto el tipo de conteo a realizar (simple o primos) como la direccion de giro del motor dc.
